...
__global__ void compute_tiles_kernel(const float *__restrict__ uvs,
  ...
  if (r_major < 32.0f) {
      ...
      for (int tile_y = start_tile_y; tile_y < end_tile_y; tile_y++) {
        ...
        if (min_right > max_left && min_bottom > max_top) {
          ...
          if (index < max_gaussians_per_tile) {
            ...
            gaussian_indices_per_tile[tile_idx * max_gaussians_per_tile +
                                      index] = gaussian_idx;
          }
        }
      }
    ...
    float obb_tl_x =
        -1 * r_major * cos_theta + r_minor * sin_theta + uvs[gaussian_idx * 2];
    float obb_tl_y = -1 * r_major * sin_theta - r_minor * cos_theta +
                     uvs[gaussian_idx * 2 + 1];

    // top_right aabb [r_major, -r_minor]
    float obb_tr_x =
        r_major * cos_theta + r_minor * sin_theta + uvs[gaussian_idx * 2];
    float obb_tr_y =
        r_major * sin_theta - r_minor * cos_theta + uvs[gaussian_idx * 2 + 1];

    // bottom_left aabb [-r_major, r_minor]
    float obb_bl_x =
        -1 * r_major * cos_theta - r_minor * sin_theta + uvs[gaussian_idx * 2];
    float obb_bl_y = -1 * r_major * sin_theta + r_minor * cos_theta +
                     uvs[gaussian_idx * 2 + 1];
    ...
    for (int tile_x = start_tile_x; tile_x < end_tile_x; tile_x++) {
      for (int tile_y = start_tile_y; tile_y < end_tile_y; tile_y++) {
        ...
        // axis0 - X axis
        float obb_min_x =
            fminf(fminf(obb_tl_x, obb_tr_x), fminf(obb_bl_x, obb_br_x));
        float obb_max_x =
            fmaxf(fmaxf(obb_tl_x, obb_tr_x), fmaxf(obb_bl_x, obb_br_x));
        if (obb_min_x > tile_right || obb_max_x < tile_left) {
          continue;
        }
        // axis1 - Y axis
        float obb_min_y =
            fminf(fminf(obb_tl_y, obb_tr_y), fminf(obb_bl_y, obb_br_y));
        float obb_max_y =
            fmaxf(fmaxf(obb_tl_y, obb_tr_y), fmaxf(obb_bl_y, obb_br_y));
...
void compute_tiles_cuda(torch::Tensor uvs, torch::Tensor sigma_image,
  ...
  TORCH_CHECK(gaussian_indices_per_tile.size(0) == n_tiles_x * n_tiles_y,
              "gaussian_indices_per_tile must have shape n_tiles x "
              "max_gaussians_per_tile");
  TORCH_CHECK(num_gaussians_per_tile.size(0) == n_tiles_x * n_tiles_y,
              "num_gaussians_per_tile must have shape n_tiles x 1");
  TORCH_CHECK(num_gaussians_per_tile.size(1) == 1,
              "num_gaussians_per_tile must have shape n_tiles x 1");
