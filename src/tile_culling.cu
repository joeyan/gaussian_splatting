...
__global__ void compute_tiles_kernel_new(
    const float* __restrict__ uvs,
    const float* __restrict__ conic,
    const int n_tiles_x,
    const int n_tiles_y,
...
__global__ void generate_gaussian_and_sort_list_kernel(
    ...
    if (output_end_idx - output_start_idx > 2) {
        printf("output_start_idx: %d, output_end_idx: %d\n", output_start_idx, output_end_idx);
    }

    for (int i = 0; i < output_end_idx - output_start_idx; i++) {
        const int tile_idx = tile_indices_per_gaussian[gaussian_idx + i];
...
std::tuple<torch::Tensor, torch::Tensor> get_sorted_gaussian_list(
    ...
    compute_tiles_kernel_new<<<gridsize, blocksize>>>(
        uvs.data_ptr<float>(),
        conic.data_ptr<float>(),
        n_tiles_x,
        n_tiles_y,
    ...
    auto result = torch::sort(sort_keys, 0, true);
