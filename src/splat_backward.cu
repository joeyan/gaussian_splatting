...
__global__ void render_tiles_backward_kernel(
    ...
    const T *__restrict__ grad_image, const int image_width,
    const int image_height, bool use_fast_exp,
  ...
  if (valid_pixel) {
    num_splats_this_pixel =
        num_splats_per_pixel[v_splat * image_width + u_splat];

#pragma unroll
    for (int channel = 0; channel < 3; channel++) {
      grad_image_local[channel] =
          grad_image[(v_splat * image_width + u_splat) * 3 + channel];
      view_dir[channel] =
          view_dir_by_pixel[(v_splat * image_width + u_splat) * 3 + channel];
    }
...
#pragma unroll
    ...
    for (int i = chunk_end - chunk_start - 1; i >= 0; i--) {
      ...
      // don't compute grad if pixel is out of bounds or this splat is after
      // saturation during forward pass
...
#pragma unroll
        ...
        grad_u = -(-b * v_diff - c * v_diff + 2 * d * u_diff) * reciprocal_det *
                 grad_mh_sq;
        grad_v = -(2 * a * v_diff - b * u_diff - c * u_diff) * reciprocal_det *
                 grad_mh_sq;
        ...
        grad_b =
            (c * common_frac - u_diff * v_diff * reciprocal_det) * grad_mh_sq;
        grad_c =
...
#pragma unroll
        for (int sh_idx = 0; sh_idx < N_SH; sh_idx++) {
          grad_sh[(channel * N_SH) + sh_idx] = cg::reduce(
              warp_cg, grad_sh[(channel * N_SH) + sh_idx], cg::plus<T>());
        }
      }
...
#pragma unroll
          for (int sh_idx = 0; sh_idx < N_SH; sh_idx++) {
            ...
            atomicAdd(grad_rgb + (gaussian_idx * 3 * N_SH) + (channel * N_SH) +
                          sh_idx,
...
void render_tiles_backward_cuda(
    ...
    torch::Tensor grad_rgb, torch::Tensor grad_opacity, torch::Tensor grad_uv,
  ...
  TORCH_CHECK(sigma_image.size(0) == N,
              "sigma_image must have the same size as uvs");
  ...
  TORCH_CHECK(splat_start_end_idx_by_tile_idx.size(0) ==
                  num_tiles_x * num_tiles_y + 1,
              "splat_start_end_idx_by_tile_idx ");
  TORCH_CHECK(num_splats_per_pixel.size(0) == image_height,
              "num_splats_per_pixel must have the same size as the image");
  TORCH_CHECK(num_splats_per_pixel.size(1) == image_width,
              "num_splats_per_pixel must have the same size as the image");
  TORCH_CHECK(final_weight_per_pixel.size(0) == image_height,
              "final_weight_per_pixel must have the same size as the image");
  TORCH_CHECK(final_weight_per_pixel.size(1) == image_width,
              "final_weight_per_pixel must have the same size as the image");

  TORCH_CHECK(grad_image.size(0) == image_height,
              "grad_image must have the same size as the image");
  TORCH_CHECK(grad_image.size(1) == image_width,
              "grad_image must have the same size as the image");
  ...
  if (uvs.dtype() == torch::kFloat32) {
    ...
    if (num_sh_coeff == 1) {
      render_tiles_backward_kernel<float, 960, 1><<<grid_size, block_size>>>(
          ...
          grad_image.data_ptr<float>(), image_width, image_height, true,
          grad_rgb.data_ptr<float>(), grad_opacity.data_ptr<float>(),
          grad_uv.data_ptr<float>(), grad_sigma_image.data_ptr<float>());
    ...
    if (num_sh_coeff == 1) {
      render_tiles_backward_kernel<double, 320, 1><<<grid_size, block_size>>>(
          ...
          grad_image.data_ptr<double>(), image_width, image_height, false,
          grad_rgb.data_ptr<double>(), grad_opacity.data_ptr<double>(),
          grad_uv.data_ptr<double>(), grad_sigma_image.data_ptr<double>());
    } else if (num_sh_coeff == 4) {
