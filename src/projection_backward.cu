...
void camera_projection_backward_cuda(torch::Tensor xyz, torch::Tensor K,
  ...
  TORCH_CHECK(uv_grad_out.size(0) == N && uv_grad_out.size(1) == 2,
              "uv_grad_out must be of shape Nx2");
  TORCH_CHECK(xyz_grad_in.size(0) == N && xyz_grad_in.size(1) == 3,
              "xyz_grad_in must be of shape Nx3");
...
__global__ void compute_projection_jacobian_backward_kernel(
  ...
  // dJ/dx = dz/du * -fx/z^2
  xyz_grad_in[i * 3 + 0] =
      jac_grad_out[i * 6 + 2] * -fx / (xyz[i * 3 + 2] * xyz[i * 3 + 2]);
  // dJ/dy = dz/dv * -fy/z^2
  xyz_grad_in[i * 3 + 1] =
      jac_grad_out[i * 6 + 5] * -fy / (xyz[i * 3 + 2] * xyz[i * 3 + 2]);
  // dJ/dz = dx/du * -fx/z^2 + dy/dv * -fy/z^2 + dz/du * 2x * fx / z^3 + dz/dv *
  // 2y * fy / z^3
  xyz_grad_in[i * 3 + 2] =
      jac_grad_out[i * 6 + 0] * -fx / (xyz[i * 3 + 2] * xyz[i * 3 + 2]) +
...
void compute_sigma_world_backward_cuda(torch::Tensor quaternions,
  ...
  TORCH_CHECK(scales.size(0) == N && scales.size(1) == 3,
              "scales must be of shape Nx3");
  TORCH_CHECK(sigma_world_grad_out.size(0) == N &&
...
void compute_sigma_image_backward_cuda(torch::Tensor sigma_world,
  ...
  TORCH_CHECK(sigma_world.size(1) == 3 && sigma_world.size(2) == 3,
              "sigma_world must be of shape Nx3x3");
  TORCH_CHECK(J.size(0) == N && J.size(1) == 2 && J.size(2) == 3,
              "J must be of shape Nx2x3");
  TORCH_CHECK(world_T_image.size(0) == 4 && world_T_image.size(1) == 4,
              "world_T_image must be of shape 4x4");
  TORCH_CHECK(sigma_image_grad_out.size(0) == N &&
                  sigma_image_grad_out.size(1) == 2 &&
                  sigma_image_grad_out.size(2) == 2,
              "sigma_image_grad_out must be of shape Nx2x2");
  TORCH_CHECK(sigma_world_grad_in.size(0) == N &&
                  sigma_world_grad_in.size(1) == 3 &&
                  sigma_world_grad_in.size(2) == 3,
              "sigma_world_grad_in must be of shape Nx3x3");
  TORCH_CHECK(J_grad_in.size(0) == N && J_grad_in.size(1) == 2 &&
                  J_grad_in.size(2) == 3,
