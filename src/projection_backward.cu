...
void camera_projection_backward_cuda (
    ...
    TORCH_CHECK(uv_grad_out.is_cuda(), "uv_grad_out must be a CUDA tensor");
    TORCH_CHECK(xyz_grad_in.is_cuda(), "xyz_grad_in must be a CUDA tensor");

    TORCH_CHECK(xyz.is_contiguous(), "xyz must be contiguous");
    TORCH_CHECK(K.is_contiguous(), "K must be contiguous");
    TORCH_CHECK(uv_grad_out.is_contiguous(), "uv_grad_out must be contiguous");
...
void compute_projection_jacobian_backward_cuda(
    ...
    TORCH_CHECK(xyz.is_cuda(), "xyz must be a CUDA tensor");
    TORCH_CHECK(K.is_cuda(), "K must be a CUDA tensor");
    TORCH_CHECK(jac_grad_out.is_cuda(), "jac_grad_out must be a CUDA tensor");
    TORCH_CHECK(xyz_grad_in.is_cuda(), "xyz_grad_in must be a CUDA tensor");

    TORCH_CHECK(xyz.is_contiguous(), "xyz must be contiguous");
    TORCH_CHECK(K.is_contiguous(), "K must be contiguous");
    TORCH_CHECK(jac_grad_out.is_contiguous(), "jac_grad_out must be contiguous");
    TORCH_CHECK(xyz_grad_in.is_contiguous(), "xyz_grad_in must be contiguous");
...
void compute_sigma_world_backward_cuda(
    ...
    TORCH_CHECK(sigma_world_grad_out.is_cuda(), "sigma_world_grad_out must be a CUDA tensor");
    TORCH_CHECK(quaternions_grad_in.is_cuda(), "quaternions_grad_in must be a CUDA tensor");
    TORCH_CHECK(scales_grad_in.is_cuda(), "scales_grad_in must be a CUDA tensor");
    ...
    TORCH_CHECK(sigma_world_grad_out.is_contiguous(), "sigma_world_grad_out must be contiguous");
    TORCH_CHECK(quaternions_grad_in.is_contiguous(), "quaternions_grad_in must be contiguous");
...
void compute_sigma_image_backward_cuda(
    ...
    TORCH_CHECK(sigma_image_grad_out.is_cuda(), "sigma_image_grad_out must be a CUDA tensor");
    TORCH_CHECK(sigma_world_grad_in.is_cuda(), "sigma_world_grad_in must be a CUDA tensor");
    TORCH_CHECK(J_grad_in.is_cuda(), "J_grad_in must be a CUDA tensor");
    ...
    TORCH_CHECK(sigma_image_grad_out.is_contiguous(), "sigma_image_grad_out must be contiguous");
    TORCH_CHECK(sigma_world_grad_in.is_contiguous(), "sigma_world_grad_in must be contiguous");
